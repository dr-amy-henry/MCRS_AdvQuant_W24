...
```{r}
load("~/Desktop/R docs/MCRS_AdvQuant_W24/CommunityDiversity_Summaries_Tutorial/S4E2e.RData")
plrich
...
#pivot wider will take the collumns and turn into rows
plrich_wide
#looks almost the same but is a df not a matrix
...
plrich_matrix <- plrich_wide[2:ncol(plrich_wide)] |> 
  as.matrix()
#this removes the species collummn 

rownames(plrich_matrix) <- plrich_wide$Species
#this now gives the rows the species names
...
```{r}
ps |> colSums()
#counts how many in each collumn
```
...
psa |> 
  ...
  group_by(Site) |> 
  count()
#pivot to make longer then filtered for rows with abund >0
psa |> 
  mutate_all(~ case_when(. > 0 ~ 1, . == 0 ~ 0)) |> 
  colSums()
#does the same thing but using the mutate function to make a present absent matrix then counts the collumns 
```
...
```{r}
library(vegan)
inv_shan <- diversity(Freshies$Count, index = "shannon", MARGIN=2)
#collumns with margin 2
