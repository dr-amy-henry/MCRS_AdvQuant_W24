...
```{r}
StomCon_Fall_Clean <- StomCon_Fall |>
  select(2:10) |>
  mutate(Date="Fall_2018")
```
...
```{r}
StomCon_Combined <- bind_rows(StomCon_Fall_Clean, StomCon_Winter_Clean)
```
